...
public class BoardDAO {
    ...
    public List<BoardVO> listContent(final int page) {
        ...
        final List<BoardVO> list = new ArrayList<BoardVO>();
        final String sql = "select B.rn, B.bno, B.title, B.mname, B.regdate, B.hit" +
                " from (select /*+index_desc(A.bno)*/ " +
                "rownum as rn, A.bno, A.title, A.mname, A.regdate, A.hit " +
                "from( select bno, title, mname, regdate, hit " +
    ...
    public void updateHit(final BoardVO vo){
     final String sql = "update tbl_board set hit = ? where bno =?";
        ...
        new QueryExecutor() {
            ...
            public void doJob() throws Exception {
                stmt = con.prepareStatement(sql);
                stmt.setInt(1, vo.getHit() + 1);
                stmt.setInt(2, vo.getBno());
                stmt.executeUpdate();
